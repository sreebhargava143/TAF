...
class CBASDataverseAndScopes(CBASBaseTest):
    ...
    def test_create_drop_dataverse(self):
        ...
        self.log.info(
            "Performing validation in Metadata.Dataverse after dropping dataverses")
        for dataverse in self.cbas_util_v2.dataverses:
    ...
    def test_create_dataverse(self):
        ...
        if self.input.param('cardinality', 0):
            for i in range(0, self.input.param('cardinality', 0)):
                name_length = self.input.param(
                    'name_{0}'.format(str(i+1)), 255)
        ...
        if not self.cbas_util_v2.create_dataverse(
            ...
            self.fail("Dataverse {0} creation passed when it should be failed".format(
                dataverse_name))
    ...
    def test_drop_dataverse(self):
        ...
        if not self.cbas_util_v2.drop_dataverse(
            ...
            self.fail("Dropping of Dataverse {0} failed".format(
                self.entity_name))
...
class CBASDatasetsAndCollections(CBASBaseTest):
    ...
    def test_enabling_disabling_analytics_collection_on_all_KV_collections(self):
        ...
        def populate_job_queue(list_of_objs, func_name):
            for obj in list_of_objs:
                if func_name == self.cbas_util_v2.validate_dataset_in_metadata:
                    jobs.put(
                        (func_name, {"dataset_name": obj.name, "dataverse_name": obj.dataverse_name}))
                elif func_name == self.cbas_util_v2.wait_for_ingestion_complete:
                    jobs.put(
                        (func_name, {"dataset_names": [obj.full_name], "num_items": obj.num_of_items}))
        ...
        populate_job_queue(
            dataset_objs, self.cbas_util_v2.enable_analytics_from_KV)
        self.cbas_util_v2.run_jobs_in_parallel(
            self.cbas_util_v2.enable_analytics_from_KV, jobs, results, 15)

        if not all(results):
            self.fail("Error while enabling analytics collection from KV")

        populate_job_queue(
            dataset_objs, self.cbas_util_v2.validate_dataset_in_metadata)
        self.cbas_util_v2.run_jobs_in_parallel(
            consumer_func, jobs, results, 15)

        if not all(results):
            self.fail("Error while validating the datasets in Metadata")

        populate_job_queue(
            dataset_objs, self.cbas_util_v2.wait_for_ingestion_complete)
        self.cbas_util_v2.run_jobs_in_parallel(
            consumer_func, jobs, results, 15)
        ...
        if self.input.param('disable_from_kv', False):
            populate_job_queue(
                dataset_objs, self.cbas_util_v2.disable_analytics_from_KV)
            self.cbas_util_v2.run_jobs_in_parallel(
                self.cbas_util_v2.disable_analytics_from_KV, jobs, results, 15)
            if not all(results):
                self.fail("Error while disabling analytics collection from KV")

            populate_job_queue(
                dataset_objs, self.cbas_util_v2.validate_dataverse_in_metadata)
            self.cbas_util_v2.run_jobs_in_parallel(
                self.cbas_util_v2.validate_dataverse_in_metadata, jobs, results, 15)
    ...
    def test_enabling_analytics_collection_from_KV(self):
        ...
        if self.input.param('create_dataverse', False) and \
            ...
            self.fail("Failed to create dataverse {0}".format(
                dataset.dataverse_name))
    ...
    def test_disabling_analytics_collection_from_KV(self):
        ...
        if self.input.param('create_dataset', False):
            ...
            if not self.cbas_util_v2.create_dataset(
                ...
                self.fail("Error creating dataset {0}".format(
                    new_dataset_name))
        ...
        if self.input.param('create_synonym', False):
            ...
            if not self.cbas_util_v2.create_analytics_synonym(
                ...
                self.fail("Error creating synonym {0}".format(
                    new_synonym_name))
    ...
    def test_dataset_and_synonym_name_resolution_precedence(self):
        ...
        if self.input.param('synonym_on_synonym', False):
            ...
            new_syn_obj = Synonym(
                self.cbas_util_v2.generate_name(
                    1), syn_obj.name, syn_obj.dataverse_name,
                dataverse_name=dv_name, synonym_on_synonym=True)
            synonym_objs.append(new_syn_obj)
    ...
    def test_docs_deleted_in_dataset_once_MaxTTL_reached(self):
        ...
        self.collectionSetUp(self.cluster, self.bucket_util,
                             self.cluster_util, buckets_spec, doc_loading_spec)
    ...
    def test_create_query_drop_on_multipart_name_secondary_index(self):
        ...
        for dataset in dataset_objs:
            ...
            if not self.cbas_util_v2.create_cbas_index(
                ...
                self.fail(
                    "Failed to create index on dataset {0}".format(dataset.name))
        ...
        def execute_function_in_parallel(func_name, num_items=None):
            ...
            for dataset in dataset_objs:
                if func_name == self.cbas_util_v2.wait_for_ingestion_complete:
                    jobs.put((func_name, {"dataset_names": [
                             dataset.full_name], "num_items": dataset.num_of_items}))
                    ...
                    if num_items is not None:
                        jobs.put(
                            (func_name, {"dataset_name": dataset.full_name, "expected_count": num_items}))
                    else:
                        jobs.put((func_name, {
                                 "dataset_name": dataset.full_name, "expected_count": dataset.num_of_items}))

            self.cbas_util_v2.run_jobs_in_parallel(
                consumer_func, jobs, results, 15, async_run=False)
    ...
    def test_KV_collection_deletion_does_not_effect_dataset_on_other_collections(self):
        ...
        collection_to_delete = CBASHelper.format_name(
            selected_dataset.get_fully_qualified_kv_entity_name(2), collection_to_delete)
        for dataset in dataset_objs:
    ...
    def test_analytics_select_rbac_role_for_collections(self):
        ...
        for bucket in self.bucket_util.buckets:
            ...
            if not status:
                self.fail(
                    "Failed to fetch all the collections in bucket {0}".format(bucket.name))
            ...
            for scope in json_parsed["scopes"]:
                ...
                for collection in scope["collections"]:
                    ...
                    dataset_names.append(
                        (dataset_name, bucket.name, scope["name"], collection["name"]))
                    ...
                    if not self.cbas_util_v2.create_dataset(
                        ...
                        self.fail(
                            "Error while creating dataset {0}".format(dataset_name))
                    count += 1
                    if count == 2:
                        break
        ...
        if self.input.param('bucket_user', False):
            self.log.info("Granting analytics_select user access to following KV entity - {0}".format(
                dataset_info[1]))
            payload = "name=" + rbac_username + \
                "&roles=analytics_select" + "[" + dataset_info[1] + "]"
            response = self.rest.add_set_builtin_user(rbac_username, payload)
        ...
        for bucket in self.bucket_util.get_all_buckets():
            for _, scope in bucket.scopes.items():
                for _, collection in scope.collections.items():
                    ...
                    bucket.scopes[scope.name].collections[collection.name].num_items += (
                        end - start)
    ...
    def test_analytics_with_parallel_dataset_creation(self):
        ...
        self.log.info("******** Loading initial docs ********")
        self.load_data(0, initial_items)
        self.log.info("******** Initial docs loading complete ********")
        final_items = self.input.param("final_items", 1000) + initial_items
        run_query = self.input.param("run_query", False)
        with ThreadPoolExecutor() as executor:
            self.log.info(
                "******** Creating datasets ********" + str(datetime.now()))
            datasets_task = executor.submit(
                self.cbas_util_v2.create_datasets_on_all_collections, bucket_util=self.bucket_util,
                cbas_name_cardinality=self.input.param('cardinality', None),
                kv_name_cardinality=self.input.param('bucket_cardinality', None), creation_methods=["cbas_dataset"])
            tasks.append(datasets_task)
            self.log.info(
                "******** Loading final docs ********" + str(datetime.now()))
            data_load_task = executor.submit(
                self.load_data, start=initial_items, end=final_items)
            ...
            if run_query:
                ...
                while not datasets_created:
                    ...
                    for dataverse in dataverses:
                        datasets_created.extend(
                            list(dataverse.datasets.keys()))
                for dataset in datasets_created:
                    query = "select sleep(count(*), 50000) from {0} where mutated=0;".format(
                        dataset)
                    handles = self.cbas_util_v2._run_concurrent_queries(
                        query, "async", int(self.input.param("num_queries", 1)), wait_for_execution=False)
        results = []
        for task in tasks:
        ...
        for dataset in datasets:
            if not self.cbas_util_v2.wait_for_ingestion_complete(
                ...
                self.fail("Data ingestion into the datasets did not complete")
        if run_query:
            self.cbas_util_v2.log_concurrent_query_outcome(
                self.cluster.master, handles)
        self.log.info(
